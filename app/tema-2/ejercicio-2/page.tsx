  ...
  return (
    <ExerciseLayout
      ...
      statement={
        <div className="space-y-2">
          ...
          <div className="overflow-x-auto"><div className="bg-white p-3 rounded text-xs font-mono grid grid-cols-8 gap-1">
            {rawData.map((d, i) => (
    ...
    >
      ...
      <StepCard stepNumber={1} title="¿Por qué agrupar datos en intervalos?" variant="explanation">
        ...
        <Card className="bg-amber-50 border-amber-200 mt-2">
          <CardContent className="p-3 text-xs space-y-1">
            <p className="font-semibold text-amber-800">Solución: agrupar en intervalos (clases)</p>
      ...
      <StepCard stepNumber={2} title="¿Cómo se clasifican los datos en intervalos?" variant="explanation">
        ...
        <Card className="bg-amber-50 border-amber-200 mt-2 mb-3">
          <CardContent className="p-3 text-xs space-y-2">
            <p className="font-semibold text-amber-800">Ejemplo concreto: intervalo (76, 78]</p>
            <p>Recorremos los 60 datos y buscamos los que cumplen: 76 &lt; x ≤ 78</p>
            <div className="bg-white rounded p-2 font-mono text-[11px]">
        ...
        <Card className="bg-blue-50 border-blue-200">
          <CardContent className="p-3 text-xs space-y-2">
            <p className="font-semibold text-blue-800 mb-2">Todos los intervalos con sus datos originales:</p>
            <div className="space-y-1.5">
              {intervalWithValues.map((d, i) => (
                <div key={i} className={`flex items-start gap-2 p-1.5 rounded ${d.values.length > 0 ? "bg-white" : "bg-gray-50 opacity-50"}`}>
                  ...
                  <Badge className={`text-[10px] shrink-0 ${d.values.length > 5 ? "bg-blue-200 text-blue-800" : d.values.length > 0 ? "bg-gray-200 text-gray-800" : "bg-gray-100 text-gray-400"}`}>
      ...
      <StepCard stepNumber={3} title="Construir los intervalos de amplitud 2" variant="calculation">
        ...
        <Card className="bg-gray-50 border mt-2 mb-3">
          <CardContent className="p-3 text-xs space-y-1">
        ...
        <Card className="bg-violet-50 border-violet-200 mt-3">
          <CardContent className="p-3 space-y-1">
            <p className="font-semibold text-xs text-violet-800">¿Por qué el primer intervalo usa [ y los demás (?</p>
            <div className="text-xs space-y-1">
              <p><Badge variant="outline" className="text-[10px] mr-1">[64, 66]</Badge> Cerrado por ambos lados: incluye el 64 y el 66. Así no perdemos el valor mínimo (65.8).</p>
              <p><Badge variant="outline" className="text-[10px] mr-1">(66, 68]</Badge> Abierto por la izquierda: NO incluye el 66 (ya está en el intervalo anterior) pero SÍ incluye el 68.</p>
        ...
        <Card className="bg-blue-50 border-blue-200 mt-3">
          <CardContent className="p-3 text-xs space-y-1">
            <p className="font-semibold text-blue-800">¿Qué es la marca de clase (<InlineMath math="x_i" />)?</p>
      ...
      <StepCard stepNumber={4} title="¿Qué son las 4 frecuencias y cómo se calculan?" variant="explanation">
          ...
          <Card className="bg-blue-50 border-blue-200 border-l-4">
            <CardContent className="p-3 space-y-1">
              <div className="flex items-center gap-2">
                <Badge className="bg-blue-200 text-blue-800 text-xs"><InlineMath math="n_i" /></Badge>
                <p className="font-semibold text-xs">Frecuencia Absoluta</p>
              </div>
              <p className="text-xs text-muted-foreground"><strong>¿Qué es?</strong> Simplemente contar: ¿cuántas personas hay en este intervalo?</p>
              <p className="text-xs"><strong>¿Cómo se calcula?</strong> Se recorren los 60 datos y se cuenta cuántos caen dentro del intervalo.</p>
              <div className="bg-white rounded p-2 mt-1">
                <p className="text-[10px]">Ej: intervalo (76, 78] → contamos los datos entre 76 y 78 → <InlineMath math="n_i" /> = 10</p>
              </div>
            </CardContent>
          </Card>

          <Card className="bg-emerald-50 border-emerald-200 border-l-4">
            <CardContent className="p-3 space-y-1">
              <div className="flex items-center gap-2">
                <Badge className="bg-emerald-200 text-emerald-800 text-xs"><InlineMath math="f_i" /></Badge>
                <p className="font-semibold text-xs">Frecuencia Relativa</p>
              </div>
              <p className="text-xs text-muted-foreground"><strong>¿Qué es?</strong> La proporción (%) que representa ese intervalo sobre el total.</p>
              <FormulaDisplay math={`f_i = \\frac{n_i}{n}`} />
              <div className="bg-white rounded p-2">
                <p className="text-[10px]">Ej: si <InlineMath math="n_i" /> = 10 y n = 60 → <InlineMath math="f_i" /> = 10/60 = 0.1667 = 16.67%</p>
              </div>
              <p className="text-[10px] text-muted-foreground">Se obtiene <strong>a partir de <InlineMath math="n_i" /></strong>. La suma de todas las <InlineMath math="f_i" /> = 1 (100%).</p>
            </CardContent>
          </Card>

          <Card className="bg-violet-50 border-violet-200 border-l-4">
            <CardContent className="p-3 space-y-1">
              <div className="flex items-center gap-2">
                <Badge className="bg-violet-200 text-violet-800 text-xs"><InlineMath math="N_i" /></Badge>
                <p className="font-semibold text-xs">Frecuencia Absoluta Acumulada</p>
              </div>
              <p className="text-xs text-muted-foreground"><strong>¿Qué es?</strong> ¿Cuántas personas hay <em>hasta</em> este intervalo (incluido)?</p>
              <FormulaDisplay math={`N_i = n_1 + n_2 + \\cdots + n_i`} />
              <div className="bg-white rounded p-2">
                <p className="text-[10px]">Ej: si los 3 primeros intervalos tienen <InlineMath math="n_i" /> = 1, 2, 5 → <InlineMath math="N_3" /> = 1+2+5 = 8 personas pesan hasta 70 kg.</p>
              </div>
              <p className="text-[10px] text-muted-foreground">Se obtiene <strong>sumando las <InlineMath math="n_i" /> anteriores</strong>. La última <InlineMath math="N_i" /> siempre es n (60).</p>
            </CardContent>
          </Card>

          <Card className="bg-rose-50 border-rose-200 border-l-4">
            <CardContent className="p-3 space-y-1">
              <div className="flex items-center gap-2">
                <Badge className="bg-rose-200 text-rose-800 text-xs"><InlineMath math="F_i" /></Badge>
                <p className="font-semibold text-xs">Frecuencia Relativa Acumulada</p>
              </div>
              <p className="text-xs text-muted-foreground"><strong>¿Qué es?</strong> ¿Qué proporción de personas pesa <em>hasta</em> este intervalo?</p>
              <FormulaDisplay math={`F_i = \\frac{N_i}{n} = f_1 + f_2 + \\cdots + f_i`} />
              <div className="bg-white rounded p-2">
                <p className="text-[10px]">Ej: si <InlineMath math="N_i" /> = 8 y n = 60 → <InlineMath math="F_i" /> = 8/60 = 0.1333 = 13.33% pesan hasta 70 kg.</p>
              </div>
              <p className="text-[10px] text-muted-foreground">Se puede calcular de <strong>dos formas</strong>: <InlineMath math="N_i/n" /> o sumando las <InlineMath math="f_i" />. La última <InlineMath math="F_i" /> siempre es 1.</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="bg-amber-50 border-amber-200 mt-3">
          <CardContent className="p-3">
            <p className="font-semibold text-xs text-amber-800 mb-1">Relación entre las 4 frecuencias:</p>
      ...
      <StepCard stepNumber={5} title="Ejemplo: calcular una fila completa paso a paso" variant="calculation">
        ...
        <div className="space-y-2 mt-2">
          <Card className="bg-blue-50 border-blue-200">
            <CardContent className="p-3 text-xs space-y-1">
              <p className="font-semibold text-blue-800">1. Frecuencia absoluta (<InlineMath math="n_i" />): contar</p>
              <p>Buscamos los datos que cumplen 68 &lt; x ≤ 70:</p>
              <p className="font-mono bg-white rounded p-1.5 text-[10px]">{intervalWithValues.find(d => d.label === "(68, 70]")?.values.join(", ") || "-"}</p>
          ...
          <Card className="bg-emerald-50 border-emerald-200">
            <CardContent className="p-3 text-xs space-y-1">
              <p className="font-semibold text-emerald-800">2. Frecuencia relativa (<InlineMath math="f_i" />): dividir entre n</p>
          ...
          <Card className="bg-violet-50 border-violet-200">
            <CardContent className="p-3 text-xs space-y-1">
              <p className="font-semibold text-violet-800">3. Frecuencia absoluta acumulada (<InlineMath math="N_i" />): sumar las anteriores</p>
          ...
          <Card className="bg-rose-50 border-rose-200">
            <CardContent className="p-3 text-xs space-y-1">
              <p className="font-semibold text-rose-800">4. Frecuencia relativa acumulada (<InlineMath math="F_i" />): dividir <InlineMath math="N_i" /> entre n</p>
      ...
      <StepCard stepNumber={7} title="c) Estimación del % entre 75 y 85 kg" variant="calculation">
        <Card className="bg-cyan-50 border-cyan-200 mb-3">
          <CardContent className="p-3 text-xs space-y-1">
            <p className="font-semibold text-cyan-800">¿Cómo estimamos un porcentaje con datos agrupados?</p>
        ...
        <div className="space-y-1 mt-2 text-sm">
          {tableData.filter(row => {
            ...
            <div key={i} className="flex justify-between px-3 py-1.5 bg-amber-50 rounded border border-amber-100">
              <span className="font-mono">{row.xi}</span>
              <span className="text-xs">
                <Badge variant="outline" className="text-[10px] mr-1"><InlineMath math="n_i" /> = {row.ni}</Badge>
                <Badge className="bg-emerald-100 text-emerald-800 text-[10px]"><InlineMath math="f_i" /> = {(row.fi * 100).toFixed(2)}%</Badge>
              </span>
            </div>
          ))}
        </div>
      ...
      <StepCard stepNumber={8} title="Resumen: ¿qué hemos aprendido?" variant="result">
        <div className="space-y-2 text-xs">
          <div className="grid md:grid-cols-2 gap-2">
            ...
            <Card className="bg-emerald-50 border-emerald-200">
              <CardContent className="p-2 space-y-1">
                <p className="font-semibold text-emerald-800">4 frecuencias</p>
            ...
            <Card className="bg-indigo-50 border-indigo-200">
              <CardContent className="p-2 space-y-1">
                <p className="font-semibold text-indigo-800">Estimación</p>
