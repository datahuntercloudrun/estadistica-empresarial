  ...
  return (
    ...
    >
      ...
      <StepCard stepNumber={1} title={<>¿Qué significan <InlineMath math="x_i" />, <InlineMath math="y_j" /> y <InlineMath math="n_{ij}" />?</>} variant="explanation">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mt-2">
          <Card className="bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800">
            <CardContent className="p-3 space-y-1">
              ...
              <p className="text-sm text-muted-foreground">Los valores posibles de la <strong>variable X</strong> (primera variable).</p>
              <p className="text-sm">En este ejercicio: <InlineMath math="x_i" /> = 1, 2, 3, 4</p>
            </CardContent>
          </Card>
          <Card className="bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800">
            <CardContent className="p-3 space-y-1">
              <Badge className="bg-emerald-200 dark:bg-emerald-800/40 text-emerald-800 dark:text-emerald-200 text-sm"><InlineMath math="y_j" /></Badge>
              <p className="text-sm text-muted-foreground">Los valores posibles de la <strong>variable Y</strong> (segunda variable).</p>
              <p className="text-sm">En este ejercicio: <InlineMath math="y_j" /> = 2, 4, 6</p>
            </CardContent>
          </Card>
        ...
        <Card className="bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800 mt-3">
          <CardContent className="p-3 text-sm space-y-1">
            <p className="font-semibold text-amber-800 dark:text-amber-200">¿Y la tabla de correlación?</p>
            ...
            <div className="mt-2 space-y-1">
              <p><Badge className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 text-sm"><InlineMath math="n_{i\cdot}" /></Badge> = suma de toda la <strong>fila</strong> i → cuántas veces aparece X = <InlineMath math="x_i" /> (sin importar Y). Es la <strong>marginal de X</strong>.</p>
              <p><Badge className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-200 text-sm"><InlineMath math="n_{\cdot j}" /></Badge> = suma de toda la <strong>columna</strong> j → cuántas veces aparece Y = <InlineMath math="y_j" /> (sin importar X). Es la <strong>marginal de Y</strong>.</p>
            </div>
          </CardContent>
        </Card>
      </StepCard>
      ...
      <StepCard stepNumber={2} title="Construir la tabla de correlación" variant="calculation">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center font-bold bg-blue-50 dark:bg-blue-950/20"><InlineMath math="n_{i\cdot}" /></TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {xValues.map((x, i) => (
                <TableRow key={x}>
                  ...
                  <TableCell className="text-center font-bold bg-blue-50 dark:bg-blue-950/20">{marginalX[i]}</TableCell>
                </TableRow>
              ))}
        ...
        <Card className="bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800 mt-3">
          <CardContent className="p-3 text-sm space-y-2">
            <p className="font-semibold text-blue-800 dark:text-blue-200">¿Cómo leer esta tabla?</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-1">
              <div className="bg-white dark:bg-gray-900 rounded p-2">
                <p><strong className="text-blue-700 dark:text-blue-300">Última columna</strong> (<InlineMath math="n_{i\cdot}" />): suma de cada fila.</p>
                <p className="text-sm text-muted-foreground">Ej: fila X=4: 1+2+1 = 4</p>
              </div>
              <div className="bg-white dark:bg-gray-900 rounded p-2">
                <p><strong className="text-emerald-700 dark:text-emerald-300">Última fila</strong> (<InlineMath math="n_{\cdot j}" />): suma de cada columna.</p>
                <p className="text-sm text-muted-foreground">Ej: columna Y=2: 1+2+1+1 = 5</p>
              </div>
            </div>
            <p className="text-sm text-muted-foreground">La casilla inferior derecha (N = {totalN}) es el total de observaciones: la suma de todas las <InlineMath math="n_{ij}" />.</p>
            <div className="bg-white dark:bg-gray-900 rounded p-2 mt-1 space-y-1">
              <p className="font-semibold text-blue-800 dark:text-blue-200">¿Para qué sirven los marginales?</p>
      ...
      <StepCard stepNumber={3} title="a) Distribuciones marginales" variant="result">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Card>
            <CardContent className="p-3">
              <p className="font-semibold text-sm mb-2 text-blue-700 dark:text-blue-300">Marginal de X</p>
              <p className="text-sm text-muted-foreground mb-2">Sumamos cada fila (todos los valores de Y para cada X):</p>
              <FormulaDisplay math={`n_{i\\cdot} = \\sum_{j} n_{ij}`} />
              <div className="bg-blue-50 dark:bg-blue-950/20 rounded p-2 text-sm space-y-0.5 mb-2">
              ...
              <p className="text-sm text-muted-foreground mb-2">La frecuencia relativa se obtiene dividiendo entre el total N = {totalN}:</p>
          ...
          <Card>
            <CardContent className="p-3">
              <p className="font-semibold text-sm mb-2 text-emerald-700 dark:text-emerald-300">Marginal de Y</p>
              <p className="text-sm text-muted-foreground mb-2">Sumamos cada columna (todos los valores de X para cada Y):</p>
              <FormulaDisplay math={`n_{\\cdot j} = \\sum_{i} n_{ij}`} />
              <div className="bg-emerald-50 dark:bg-emerald-950/20 rounded p-2 text-sm space-y-0.5 mb-2">
              ...
              <p className="text-sm text-muted-foreground mb-2">Dividimos entre N = {totalN}:</p>
        ...
        <Card className="bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800 mt-3">
          <CardContent className="p-3 text-sm space-y-2">
            <p className="font-semibold text-amber-800 dark:text-amber-200">Verificación</p>
            ...
            <div className="bg-white dark:bg-gray-900 rounded p-2 space-y-1">
              <p className="font-semibold text-amber-800 dark:text-amber-200">¿Por qué tienen que ser iguales?</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-1">
                <div className="bg-blue-50 dark:bg-blue-950/20 rounded p-2">
                  <p><strong className="text-blue-700 dark:text-blue-300">Sumar todos los <InlineMath math="n_{i\cdot}" /></strong></p>
                  <p className="text-sm text-muted-foreground">= sumar fila por fila. Cada fila contiene todas las observaciones de un valor de X. Al sumar todas las filas, has contado <strong>todas</strong> las observaciones → N.</p>
                </div>
                <div className="bg-emerald-50 dark:bg-emerald-950/20 rounded p-2">
                  <p><strong className="text-emerald-700 dark:text-emerald-300">Sumar todos los <InlineMath math="n_{\cdot j}" /></strong></p>
                  <p className="text-sm text-muted-foreground">= sumar columna por columna. Cada columna contiene todas las observaciones de un valor de Y. Al sumar todas las columnas, has contado <strong>todas</strong> las observaciones → N.</p>
                </div>
              </div>
              <p className="text-sm text-muted-foreground mt-1">Es como contar personas sentadas en un aula: puedes contar fila por fila o columna por columna, pero el total siempre es el mismo.</p>
              <div className="bg-gray-50 dark:bg-gray-800 rounded p-2 mt-1">
                <p className="text-sm">En nuestro caso: <InlineMath math="n_{1\cdot} + n_{2\cdot} + n_{3\cdot} + n_{4\cdot}" /> = {marginalX.join(" + ")} = {totalN} &nbsp;y&nbsp; <InlineMath math="n_{\cdot 1} + n_{\cdot 2} + n_{\cdot 3}" /> = {marginalY.join(" + ")} = {totalN} ✓</p>
              </div>
            </div>
      ...
      <StepCard stepNumber={4} title="b) Distribución de Y condicionada a X = 3" variant="calculation">
        ...
        <p className="text-sm text-muted-foreground mb-2">Miramos solo la fila de X = 3 (resaltada en la tabla del paso 2). Las frecuencias son:</p>
        <div className="bg-blue-50 dark:bg-blue-950/20 rounded p-2 text-sm mb-3">
        ...
        <p className="text-sm text-muted-foreground mb-2">Dividimos cada frecuencia de la fila entre el total de la fila (<InlineMath math="n_{3\cdot}" /> = {marginalX[2]}), NO entre N = {totalN}:</p>
        <FormulaDisplay math={`f(y_j \\mid X=3) = \\frac{n_{3j}}{n_{3\\cdot}} = \\frac{n_{3j}}{${marginalX[2]}}`} />

        <div className="bg-violet-50 dark:bg-violet-950/20 rounded p-2 text-sm space-y-0.5 mb-3">
          {yValues.map((y, j) => {
        ...
        <Card className="bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800 mt-3">
          <CardContent className="p-3 text-sm space-y-1">
            <p className="font-semibold text-amber-800 dark:text-amber-200">¿Cómo interpretar este resultado?</p>
      ...
      <StepCard stepNumber={5} title="c) Distribución de X condicionada a Y = 2" variant="calculation">
        ...
        <p className="text-sm text-muted-foreground mb-2">Miramos solo la columna de Y = 2. Las frecuencias son:</p>
        <div className="bg-emerald-50 dark:bg-emerald-950/20 rounded p-2 text-sm mb-3">
        ...
        <p className="font-medium text-sm">Paso 2: Calcular las frecuencias relativas condicionadas</p>
        <p className="text-sm text-muted-foreground mb-2">Dividimos cada frecuencia de la columna entre el total de la columna (<InlineMath math="n_{\cdot 1}" /> = {marginalY[0]}):</p>
